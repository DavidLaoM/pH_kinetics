...
temp_simResult_REV = cell(DFs,numpH_REV);
    ...
    % creating interpolated Vmax array
    pH_fwd = data.FWD.pH(:,4);
    pH_rev = data.REV.pH(:,4);
    vmax_fwd = data.FWD.Vmax(:,4);
    vmax_rev = data.REV.Vmax(:,4);
    vmax_fwd_interp = interp1(pH_fwd,vmax_fwd,pH_rev,'pchip');
...
%         data.REV.chosenVmax = data.REV.Vmaxs(1,4)/data.REV.DF(1,i); % vmax from the highest DF is taken and then divided
        data.REV.chosenVmaxFWD = vmax_fwd_interp(j)./data.REV.DF(1,i);
        data.REV.chosenVmaxREV = vmax_rev(j)./data.REV.DF(1,i);
        ...
        % simulate metabolites
            ...
            setup.REV.directionFWD = 0;
            setup.REV.directionREV = 1;
        [simResult] = simSys_gapdhFWDREV(xassay,data.REV,setup.REV);
        % calculation of reaction rates
        [vObs,~] = calcRates_gapdhFWDREV(xassay,simResult,data.REV,setup.REV);   
...
%                 data.chosenVmax = data.Vmaxs(1,4)/data.DF(1,i); % vmax from the highest DF is taken and then divided
        data.FWD.chosenVmaxFWD = vmax_fwd(j)./data.REV.DF(1,i);
        data.FWD.chosenVmaxREV = vmax_rev_interp(j)./data.REV.DF(1,i);
        ...
        % simulate metabolites
            ...
            setup.FWD.directionFWD = 1;
            setup.FWD.directionREV = 0;
        [simResult] = simSys_gapdhFWDREV(xassay,data.FWD,setup.FWD);
        % calculation of reaction rates
        [vObs,~] = calcRates_gapdhFWDREV(xassay,simResult,data.FWD,setup.FWD);   
        % cost function (NADHexp + vGAPDHr)
        simTime = simResult.t;
        simMet = simResult.y(:,obsMet);
        simRate = vObs;
...
switch costfun
    case 3
        ...
        % Haldande error
        keq_fwd = data.FWD.KeqGAPDH;
        V_fwd = vmax_fwd' .* 10 .^ x_temp(49:60);
        Kp1 = 10.^x_temp(13:24).*1.18;
        Kp2 = 10.^x_temp(37:48).*0.022;
        V_rev = vmax_rev_interp' .* 10 .^ x_temp(61:72);
        Ks1 = 10.^x_temp(1:12).*2.48;
        Ks2 = 10.^x_temp(25:36).*2.92;
        errorHaldane = keq_fwd - (V_fwd .* Kp1 .* Kp2) ./ (V_rev .* Ks1 .* Ks2); % keq = (V+ .* Kp1 .* Kp2) ./ (V- .* Ks1 .* Ks2);  
