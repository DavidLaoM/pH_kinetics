...
% % % %     inBetween = [tempResult{8}.y(:,7)', fliplr(tempResult{2}.y(:,7)')];
    x2 = [tempResult{15}.t', fliplr(tempResult{11}.t')];
    inBetween = [tempResult{15}.y(:,7)', fliplr(tempResult{11}.y(:,7)')];
...
% %%
% % load('new_tempRes_figure_Keq_hxk_mod.mat');
% vmax_prof = zeros(1,length(keqvalsTested));
% for i = 1:length(keqvalsTested)
%     vmax_prof(i) = max(abs(tempResult2{i}{2}));
% %     vmax_prof(i) = max(abs(tempResult2{i}{1}));
% end
% 
% figure
% semilogx(keqvalsTested, vmax_prof,'k.-')
