...
tspan = [0 300];
options = [];
[t,y] = ode15s(@ode_gapdh_pH_5,tspan,y0,options,p);
r = calcRates_gapdh_pH_5(y,p);
...
% model
function v = ode_gapdh_pH_5(tspan,y0,p)
...
vGAPDHrev = p.gapdh.Vf .*...
    ...
    (BPG .* NADH - GAP .* NAD ./ p.gapdh.Keq_rev) ./ ...
...
% calcRates
function r = calcRates_gapdh_pH_5(y,p)
% recall initial concentrations
P3G = y(:,1);
ATP = y(:,2);
BPG = y(:,3);
