function [error,simResult] = costfun_Kmfixed_simOutput(x_temp,data,setup)
        ...
        for j = 1:numpH
            ...
            if isfield(setup,'KeqPSA')
                if setup.KeqPSA == 1
                    nPSA = setup.nPSA;
                    % inputs to be selected
                    data.chosenKeqGAPDH = data.KeqGAPDH(nPSA);
                    data.chosenKeqPGK = data.KeqPGK(nPSA);
%                     data.chosenKeqPGK = data.KeqPGK(6);
                end
            end            
