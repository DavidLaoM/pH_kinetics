function [error,simResult] = simRes_costfun_Kmfixed_2(x_temp,data,setup)
...
% % % %             for temp12 = 2 % updated for the simplified topology on 2020 - 09 -30
% % % %             switch j
% % % %                 case 1
% % % %                     xassay = zeros(1,1);
% % % %                     xassay(1) = x_temp(1);
...
% % % %                     subplot(3,4,j)
% % % %                     for i = DFstudy
% % % %                         plot(data.tempTime{i}, simGAPDHr{i,j},'-','LineWidth',2)
% % % %                         hold on
% % % %                         plot(data.tempTime{i}, expGAPDHr{i,j},'k.','MarkerSize',4)
% % % %                     end
% % % %                     if j == numpH
% % % %                         suptitle('GAPDHrev reaction rate [mM s^{-1}] vs assay time [s]')
% % % %                     end       
% % % %                     
...
%             data.KeqPGK = setup.pH_Keq_pgk;
            ...
            % Mode 1. still keeping kms, variable
            if setup.Kmvariable == 1
            for temp11 = 2
            switch j
                case 1
                    xassay = zeros(1,5);
...
%                 data.chosenVmax = data.Vmaxs(1,4)/data.DF(1,i); % vmax from the highest DF is taken and then divided
                ...
                simTime = simResult.t;
                simMet = simResult.y(:,obsMet);
                simRate = vObs; simResult.v = simRate;
                
                timeInterp = [data.tempTime{i}; setup.addedTime];
                simNADH{i,j} = interp1(simTime,simMet,timeInterp,'pchip');
                simGAPDHr{i,j} = interp1(simTime,simRate,timeInterp,'pchip');
                expNADH{i,j} = data.NADH{i};
                expGAPDHr{i,j} = data.Vprofs{i};
                simResult.vObs = vObs;
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        ...
        simResult.simGAPDHr = simGAPDHr;
        simResult.expNADH = expNADH;
        simResult.expGAPDHr = expGAPDHr;
        simResult.data = data;
        simResult.DFstudy = DFstudy;
...
%                 selectedVal = 1:4:121;                
%                 timeInterp = data.tempTime{i};
                timeInterp = [data.tempTime{i}; setup.addedTime];
                simPEP{i,j} = interp1(simTime,simMet,timeInterp,'pchip');
                simENO{i,j} = interp1(simTime,simRate,timeInterp,'pchip');
                expPEP{i,j} = data.PEP{i};
                expENO{i,j} = data.Vprofs{i};
            end
        end
        % simResult save
        simResult.simPEP = simPEP;
        simResult.simENO = simENO;
        simResult.expPEP = expPEP;
        simResult.expENO = expENO;
        simResult.data = data;
        simResult.DFstudy = DFstudy;
...
%             simRes = cell(numpH,DFstudy);
            for i = DFstudy
                ...
                simTime = simResult.t;
                simMet = simResult.y(:,obsMet);
                simRate = vObs(:,3); simResult.v = simRate;
                % locate values in structure
                timeInterp = [data.tempTime{i}; setup.addedTime];
                simNADH{i,j} = interp1(simTime,simMet,timeInterp,'pchip');
                simPFK{i,j} = interp1(simTime,simRate,timeInterp,'pchip');
                expNADH{i,j} = data.NADH{i};
        ...
        % simResult save
        simResult.simNADH = simNADH;
        simResult.simPFK = simPFK;
        simResult.expNADH = expNADH;
        simResult.expPFK = expPFK;
        simResult.data = data;
        simResult.DFstudy = DFstudy;        
